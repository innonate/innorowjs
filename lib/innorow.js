...
hall.on('alert', function (level) {
  if (level == 1) {
    ...
    if (stopWatchState == 'ON' || stopWatchState == 'NOT_STARTED'){
      ...
      cyclePayload.split = fiveHundredSplit(30);
    }
...
var currentRpm = function(sensitivity=60){
  ...
  relevant = 0;
  starting_index = cycles.length - 1;
  do {
     starting_index -= 1;
     relevant += 1
  } while (cycles[starting_index] >= minuteAgo);
...
var fiveHundredSplit = function(sensitivity=500.0){
  ...
  last_cycle_index = cycles.length - 1
  fhmago = cycles[(last_cycle_index - rpms_per_fhun)]
  if (fhmago == undefined) {
    clockValue = 0
  } else {
    clockValue = Math.round((cycles[last_cycle_index] - fhmago)*multiplier)
  }
  return clockFace(clockValue);
}
...
var detectStroke = function(time){
  ...
  for( var i = 0; i < (chunkSize*2); i++ ){
    diff = (cycles[last_cycle_index - i] - cycles[last_cycle_index - (i+1)]);
    if (isNaN(diff)){
      // do nothing
    } else {
      if (i>=chunkSize){
  ...
  currentlyAccelerating = (Math.abs(recentChunk) < Math.abs(previoChunk)) ? true : false
...
var strokeRate = function(sensitivity=60){
  ...
  relevant = 0;
  last_stroke_index = strokes.length - 1;
  do {
     last_stroke_index -= 1;
     relevant += 1
  } while (strokes[last_stroke_index] >= minuteAgo);
  // return relevant.length * (60/sensitivity)
  return relevant * (60/sensitivity)
}
  ...
  if ((stopWatchState == 'NOT_STARTED') || (stopWatchState == 'PAUSED')){
    ...
    var theStopWatch = setInterval(function(){
      if (stopWatchState == 'ON') {
        ...
        io.emit('stopwatch time', clockFace(timeElapsed));
      } else {
...
function clockFace(seconds){
  if (seconds <= 60) {
    ...
    return (minutes + ':' + pad(remainder_seconds, 2))
  }
}
